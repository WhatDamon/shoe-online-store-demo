...
// PDP 页面锚点：FAB 打开时把「当前正在看的鞋」带给助手——此前 FAB 固定 open('shopping', null)，
...
const pageProduct = createStore<PageProductRef | null>({ initial: null, serverSnapshot: null })

/** PDP 挂载/换款时写入；卸载（离开 PDP）时由组件清回 null。换款后旧值被新值覆盖。 */
export function registerPageProduct(ref: PageProductRef | null): void {
  // 调用方每次渲染都可能传新的对象字面量，按 handle/title 比对，同款重复注册不写入。
  const prev = pageProduct.getSnapshot()
  if (prev?.handle === ref?.handle && prev?.title === ref?.title) return
  pageProduct.set(ref)
}

/** FAB 在点击时读取（事件时机，无需订阅/useSyncExternalStore）。 */
export function getPageProductSnapshot(): PageProductRef | null {
  return pageProduct.getSnapshot()
}
