...
3. **PDP 通用护理海报 + Buy Button 测试款整体回退（用户决策 2026-09-07）**：本阶段不做产品→海报映射（其余海报延后），仅落地每 PDP 通用的护理说明海报。① **移除测试款 3d-shoes / evo-30**：seed 与本地 DB 删除该演示商品（Buy Button 唯一宿主），Buy Button 集成全链路回退 —— 删除 ShopifyBuyButton 组件、shopify-buy.ts 库及其测试、ProductActions 的 storeLive/buySlot 分支、page 的价格隐藏逻辑与 .env/.env.example/README 的 SHOPIFY_BUY_BUTTON 段落（SHOPIFY_DOMAIN / SHOPIFY_STOREFRONT_TOKEN 预留行保留，spec #14 远期仍可选）；回退后全部 PDP 恢复 demo 价 + “Available soon” 占位购买条，目录回归纯 29 款真实萨洛丁。② **护理海报 ProductCareInstructions**（~/Desktop/Evoloop/Posters/，26MB PNG 压缩为 WebP 入库）：作为站点通用资产（非产品字段），每个 PDP 详情区提供查看入口，点击以弹窗/全屏遮罩方式展示护理海报大图；克制呈现，不占 PDP 首屏。（记录于 2026-09-07 产品映射讨论）
