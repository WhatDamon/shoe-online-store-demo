...
1. **products 表：商品数据入库（用户决策 2026-09-06）**：目录运行时源由“内存 seedProducts”迁到 SQL 表 `products` —— DB 为运行时源，启动/首次访问时表空则自动从导入层（supplier.json + seed.ts 策展）灌种一次；此后 /shop 列表、PDP、检索一致从 DB 读取（改价/改集合/上下架只需写该表；JSON 降级为“导入源”，不再作为运行时真源）。**存储形态（用户决策）**：规范列 + JSON 副列 —— id(PK)/handle(UNIQUE)/title/subtitle/description/price_amount/currency/product_type/collections(JSON)/sizes(JSON)/colors(JSON)/features(JSON)/tags(JSON)/construction(JSON)/visual(JSON)/images(JSON)/fit_notes/created_at；两方言同构（SQLite text · Postgres text/double），无 jsonb/方言分支；读写经单一编解码映射模块。**方言范围（用户决策）**：双驱动都加（schema.ts 与 schema-postgres.ts 同构 + parity 契约测试继续约束）。运行选择开关：`CATALOG_SOURCE=db|seed|shopify`（默认 `db`；测试环境 `seed` 保持既有单元测试的纯内存语义；`SHOPIFY_*` 配置仍优先生效走 stub）。自动建表沿用 CREATE TABLE IF NOT EXISTS；embedding 缓存键仍为 Product.id（evo-XX，行解码后不变，不回退）。过滤/排序继续内存执行（29 行规模），DB 负责持久化与后续扩展查询。

2. **Vercel 部署修正：Next 运行时必须去 Bun 化（用户决策 2026-09-07）**：Vercel 构建机（Linux x64 baseline）上 `bun --bun next build` 使 Next 跑在 Bun 运行时，Bun 1.3.14 在构建收尾 SIGILL（编译本身全过，纯 Bun 崩溃）；且 Vercel Next 函数运行时是 **Node**，`bun:sqlite`（Bun 专有协议）在 Node 下既不能构建也不能运行——此前“Bun-only 运行时”的本地决策与 Vercel 部署不兼容。**修正**：dev/build/start 脚本去掉 `bun --bun`，Next 一律由 Node 执行（next bin shebang 即 node，`bun run dev` 同样落 Node），SIGILL 根除；sqlite 驱动 `bun:sqlite` → **`better-sqlite3`**（drizzle 官方驱动 `drizzle-orm/better-sqlite3`，Node 原生、Vercel 友好、Bun 经 N-API 亦可）；删除测试专用 bun-sqlite-compat shim 与 vitest 的 `bun:sqlite` alias/inline hack；`next.config.ts` 增 `serverExternalPackages: ['better-sqlite3']`（原生 .node 不可打包需 external）；Bun 仅作包管理器。**双驱动保留**：`DB_DRIVER=sqlite|postgres` + `DATABASE_URL` 逻辑零改动，PG 代码路径原样待用。**Vercel 现阶段不接 PG**（用户决策）：演示走 sqlite 落 `/tmp/evoloop.db`（Vercel 函数仅 /tmp 可写且实例态无持久盘 → products 表每实例冷启动自动灌种、AI 用量/护栏退化为单实例近似，P7 既有记录）；或 `CATALOG_SOURCE=seed` 纯静态目录（DB 仅剩护栏计量）。构建机 SSG（PDP 静态页）照常灌库出页面。记录于 2026-09-07 Vercel 部署失败诊断。
19. **PDP 通用护理海报 + Buy Button 测试款整体回退（用户决策 2026-09-07）**：本阶段不做产品→海报映射（其余海报延后），仅落地每 PDP 通用的护理说明海报。① **移除测试款 3d-shoes / evo-30**：seed 与本地 DB 删除该演示商品（Buy Button 唯一宿主），Buy Button 集成全链路回退 —— 删除 ShopifyBuyButton 组件、shopify-buy.ts 库及其测试、ProductActions 的 storeLive/buySlot 分支、page 的价格隐藏逻辑与 .env/.env.example/README 的 SHOPIFY_BUY_BUTTON 段落（SHOPIFY_DOMAIN / SHOPIFY_STOREFRONT_TOKEN 预留行保留，spec #14 远期仍可选）；回退后全部 PDP 恢复 demo 价 + “Available soon” 占位购买条，目录回归纯 29 款真实萨洛丁。② **护理海报 ProductCareInstructions**（~/Desktop/Evoloop/Posters/，26MB PNG 压缩为 WebP 入库）：作为站点通用资产（非产品字段），每个 PDP 详情区提供查看入口，点击以弹窗/全屏遮罩方式展示护理海报大图；克制呈现，不占 PDP 首屏。（记录于 2026-09-07 产品映射讨论）
