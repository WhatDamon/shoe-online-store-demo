/**
 * Evoloop product visual — vanilla port of the Next.js ProductVisual
 * (src/components/shop/product-visual.tsx). Deterministic inline SVG,
 * three views (side / sole / detail), patterns lattice | wave | honeycomb.
 * No network, no randomness — same geometry as the demo build.
 *
 * Built exclusively with DOM APIs (createElementNS) — no innerHTML, so
 * metafield-sourced values are never parsed as markup.
 *
 * Usage: mount([data-evo-visual]) reads data-pattern / data-density /
 * data-palette (JSON array) / data-accent / data-name. A root with
 * data-view renders that single view (cards); otherwise the PDP gallery
 * (Side / Sole / Detail tabs).
 */
;(function () {
  'use strict'
  ...
  function svgNode(view, cfg) {
    ...
    var S = cellSize(view === 'detail' ? 16 : 13, density, view)
    ...
    svg.appendChild(el('defs', null)).appendChild(patternNode(id, pattern, palette, accent, S))
    var g = el('g', null)
    var fill = 'url(#' + id + ')'

    if (view === 'side') {
      g.appendChild(el('path', { d: SIDE_UPPER, fill: palette[0] }))
      ...
      g.appendChild(el('path', { d: SOLE_OUTER, fill: palette[0] }))
      g.appendChild(el('path', { d: SOLE_OUTER, fill: fill }))
      g.appendChild(el('ellipse', { cx: 158, cy: 82, rx: 38, ry: 28, fill: accent, opacity: 0.85 }))
      g.appendChild(el('ellipse', { cx: 84, cy: 78, rx: 30, ry: 22, fill: accent, opacity: 0.6 }))
      g.appendChild(
        el('ellipse', {
          cx: 158,
          cy: 82,
      ...
      g.appendChild(el('rect', { x: 24, y: 24, width: 112, height: 112, rx: 14, fill: palette[0] }))
  ...
  function cfgFrom(el) {
    ...
    try {
      var parsed = JSON.parse(el.getAttribute('data-palette') || '')
      if (Array.isArray(parsed) && parsed.length >= 2) palette = parsed
    } catch (e) {
      /* keep defaults */
    }
    var density = parseFloat(el.getAttribute('data-density') || '')
    if (!isFinite(density)) density = DEFAULTS.density
    return {
      pattern: (el.getAttribute('data-pattern') || DEFAULTS.pattern).toLowerCase(),
  ...
  function renderSingle(el) {
    var view = el.getAttribute('data-view') || 'side'
    el.replaceChildren(svgNode(view, cfgFrom(el)))
  }
  ...
  function renderGallery(el) {
    ...
    VIEW_LABELS.forEach(function (v) {
      ...
      b.setAttribute('data-evo-view-btn', v.view)
    ...
    function paint() {
      ...
      VIEW_LABELS.forEach(function (v) {
        var active = v.view === current
        buttons[v.view].setAttribute('aria-pressed', String(active))
        buttons[v.view].classList.toggle('is-active', active)
      })
    }

    el.replaceChildren(canvas, tabs)
    paint()
  }
  ...
  function mountAll(root) {
    ...
    Array.prototype.forEach.call(nodes, function (node) {
      if (node.getAttribute('data-view') !== null) renderSingle(node)
      else renderGallery(node)
    })
  }

  window.EvoloopVisual = { mount: mountAll, renderSingle: renderSingle }
