...
def parse_segments(seg_text: str) -> tuple[list[dict], list[int], str]:
    ...
    label = 'both' if genders == {'men', 'women'} else ('men' if 'men' in genders else 'women')
...
def main() -> int:
    ...
    for am in re.finditer(r'<xdr:((?:oneCell|twoCell|absolute)Anchor)[^>]*>(.*?)</xdr:\1>', drawing, re.S):
        block = am.group(0)
        f = re.search(r'<xdr:from>.*?<xdr:col>(\d+)</xdr:col>.*?<xdr:row>(\d+)</xdr:row>', block, re.S)
        embed = re.search(r'r:embed="(rId\d+)"', block)
        if f and embed:
            try:
                anchors.append((int(f.group(2)), int(f.group(1)), rels.get(embed.group(1), '')))
            except ValueError:
                continue
    ...
    for r in range(3, ws.max_row + 1):
        ...
        colors = [{'en': color_of(c)[0], 'hex': color_of(c)[1], 'cn': c.strip()} for c in colors_raw]
        segments, sizes, gender = parse_segments(seg_text)
        imgs = [m for (rr, _c, m) in anchors if rr == r - 1] or []
        if imgs:
            seen = set()
            imgs = [x for x in imgs if not (x in seen or seen.add(x))]  # 去重（同款重复锚定）
